...
//  int EVP_CipherFinal(EVP_CIPHER_CTX *ctx, unsigned char *outm, int *outl);
    ...
    // Initializes the cipher (ChaCha20) in ENCRYPT mode with the given key/IV.
    // Calls handle_errors() and aborts if it fails.
    if(!EVP_CipherInit(ctx, EVP_chacha20(), key, iv, ENCRYPT))
        handle_errors();

    // Declares variables to store the output length and buffers for input/output data.
    int length;
    ...
    // Reads data in chunks, encrypts it using EVP_CipherUpdate, and writes the result to f_out.
    // Prints the number of bytes read and the resulting ciphertext length per cycle.
    // If writing fails, prints an error and aborts.
    ...
    // Finalizes the encryption process to handle any remaining block data.
    // If it fails, handle_errors() is called.
    if(!EVP_CipherFinal_ex(ctx, ciphertext, &length))
        handle_errors();

    // Prints how many bytes were produced during finalization.
    // Then writes those bytes to the output file, checking for errors.
    printf("lenght=%d\n", length);
    if(fwrite(ciphertext, 1, length, f_out) < length) {
    ...
    // Informs the user that the file is encrypted.
    printf("File encrypted!\n");

    // Cleans up OpenSSL data and error strings before exiting.
