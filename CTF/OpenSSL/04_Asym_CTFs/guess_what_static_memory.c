 ...
 *   Writes the result into the provided output buffer.
 *
 *   input: the input colon-separated string (e.g., "00:ab:cd:ef")
 *   output: a buffer large enough to hold the hex digits only (e.g., "00abcdef")
 */
 ...
 *   output: buffer for the new string with colons (e.g., "00:ab:cd:ef")
 */
...
int main(void) {
    ...
    */
    ...
    int target_len = strlen(p_hex);  // p_hex's length (should be even, with a leading "00")
    ...
    // Prepare a buffer for the colon-inserted representation.
